...
2. 敏感数据默认 handle 化，明文释放必须走显式 declassification。
3. 采用事务化 `PREVIEW -> COMMIT` 与 MPC 产出提交证据。
4. 副作用仅由独立 Executor 执行，缺少双策略证明即 fail-closed。
5. Executor 在提交前强制执行 sanitize patch（Patch-Carrying Egress Integrity）。
6. 通过 Capsule 约束恶意 skill 的本地绕过路径（文件/进程/网络）。
7. 通过 delegation + 双主体能力交集 + `hctx` 绑定处理多主体联邦调用。
...
任何成功执行的副作用请求，必须携带来自 `P0/P1` 的有效 commit proofs，且绑定同一 `(action_id, program_id, request_sha256)`，其中 `request_sha256` 绑定 `(v, intent_id, caller, session, inputs, auth_context)`。

### 3.2 SM：Secret Myopia（秘密近视）

默认路径下不向不可信运行时返回敏感明文，仅返回 opaque handle；  
明文释放需显式满足去密级约束（如用户确认），并可纳入预算与审计。

### 3.3 PEI：Patch-Carrying Egress Integrity（补丁携带外发完整性）

当策略输出包含 sanitize patch 时，Executor 必须在提交前强制应用补丁；  
任何绕过补丁直接提交的路径都应被拒绝。
...
### 3.5 DAS：Delegation-Aware Binding Safety（委托感知绑定安全）

多主体调用中，delegation 与联邦 ingress 身份必须绑定进 `ReqHash(rho)` 的 `hctx`；  
跨主体、跨会话或跨委托复用 PREVIEW 证据应在 COMMIT 被拒绝。

### 3.6 SAP：Single-Auditor Privacy（单审计方隐私，可选部署）
...
3. **补丁执行一致性问题**  
   将 sanitize patch 纳入 Executor 强制执行路径，避免“预览通过补丁、提交绕过补丁”。
...
7. **多主体重放与混淆代理问题**  
   通过 delegation、双主体能力交集与 `hctx` 绑定，阻断跨主体复用提交证据。
...
1. 计算 canonical `request_sha256` 绑定（覆盖 `(v, intent_id, caller, session, inputs, context)`，其中 context 在存在时含 `external_principal/delegation_jti`）；
...
1. 校验 `tx_id` 的 caller/session/TTL/auth-context 绑定；
2. Executor 校验双 MAC proof 与哈希绑定；
3. 重构输出位（`allow_pre/need_confirm/patch`）；
4. 在 Executor 内按 proof 指示强制应用 sanitize patch 后执行副作用。

### 5.5 Patch-Carrying Egress Integrity（PEI）

- PREVIEW 产物包含 patch 约束（patch id 或 patch 参数摘要）。
- COMMIT 前由 Executor 执行 `sanitize(payload, patch)` 并再进行 effect。
- 若 proof 中声明必须补丁但请求 payload 未满足补丁语义，直接 fail-closed。

### 5.6 Skill Ingress 事务门控
...
### 5.9 Delegation + 双主体交集 + 联邦绑定（DAS）

- 委托令牌采用 `delta=(iss,sub,scope,session,exp,jti)` 并校验签名、会话、scope、撤销状态。
- 有外部主体时，实际权限采用 `caps(caller) ∩ caps(external_principal)`，防止混淆代理升权。
- 将 `external_principal` 与 `delegation_jti` 纳入 `hctx`，并纳入 `ReqHash(rho)` 绑定，防止跨主体重放。
...
1. 构建了针对 Agent 副作用的可验证不可绕过执行线（NBE，双证明执行器）。
2. 实现了 handle-first + 显式去密级的秘密近视语义，并支持补丁携带式提交完整性（SM + PEI）。
3. 将 skill install/enable 纳入与 egress 同等级事务控制，并通过 capsule 合同约束本地绕过路径（SCS）。
4. 实现 delegation、双主体能力交集与 `hctx` 绑定，补齐多主体联邦调用的抗重放语义（DAS）。
5. 在可选部署下提供基于 2-server PIR 的单审计方隐私外包口径（SAP）。
...
3. Capsule 为 MVP 级，跨平台硬化与内核对抗不在当前完成范围，SCS 结论依赖合同验证通过。
4. SAP 为可选模式，依赖双策略服务不合谋与固定形状泄露口径。
5. `request_sha256` 绑定依赖 canonical 序列化一致性与部署侧密钥/撤销存储正确性。
6. 数据集规模与策略语言覆盖仍可继续扩展。
...
| R-15 | 策略外包 transcript 泄露 | 未整形 PIR transcript 可分离（`mi_bits=0.4143`, `map_acc=0.5145`） | `artifact_out_compare/leakage_sweep/leakage_model_sweep.json` |
| R-16 | 常量形状抑制侧信道 | 全整形后 `pir.mi_bits=0.0`, `pir.map_acc=0.3455`，`mpc.mi_bits≈0`，接近 chance | `artifact_out_compare/leakage_sweep/leakage_model_sweep.json` |
