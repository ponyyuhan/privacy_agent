...
both are absent). Define the request binding hash:
\[
\mathsf{ReqHash}(\mathsf{intent\_id},\mathsf{caller},\mathsf{session},\mathsf{inputs},\mathsf{hctx}) :=
...
consistent, and executor recomputes request hash including $\mathsf{hctx}$.
...
\paragraph{Proof sketch.}
Changing caller, session, or hash-context changes the bound request hash.
