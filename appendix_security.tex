...
no external side effect can be committed without dual valid request bound commit proofs, and a single
policy server learns no more than the allowed leakage function under stated assumptions.
The first clause is formalized by Theorem \ref{thm:nbe}.
The second clause is formalized by Theorem \ref{thm:sap}.
...
\midrule
NBE & No unauthorized side effects & MAC EUF-CMA, hash binding consistency, replay store \\
SM & No plaintext flow to runtime & Handle opacity, explicit declassification only \\
SAP & Single server transcript privacy & PIR single server security, MPC HBC privacy, non-collusion \\
...
The corresponding machine checkable JSON schema for the commit evidence object is
\texttt{spec/secureclaw\_executor\_accept\_v1.schema.json}.
Additional semantic constraints used by the executor acceptance predicate are specified in
\texttt{spec/secureclaw\_accept\_predicate\_v1.json} and are machine checked by
\texttt{scripts/verify\_accept\_predicate.py}.
...
cryptography and confirms that the event $\mathsf{Bad}_{\mathsf{NBE}}$ cannot occur when the executor
enforces $\mathsf{Accept}$ and the interface fields match the schema.
