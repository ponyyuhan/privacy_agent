...
\paragraph{Single reviewable claim.}
SecureClaw enforces a non bypassable side effect commit line with dual proof verification and request binding, while outsourcing policy checks with single auditor privacy up to an explicit leakage function.
...
\subsection{Multi agent federation and delegated authority}
\label{sec:multiagent}

SecureClaw supports cross agent requests by introducing a delegated authority tuple
\[
\delta := (\mathsf{iss}, \mathsf{sub}, \mathsf{scope}, \mathsf{session}, \mathsf{exp}, \mathsf{jti})
\]
that is signed with an operator managed key.
Here $\mathsf{iss}$ is an external principal, $\mathsf{sub}$ is the internal actor pattern, $\mathsf{scope}$ is an intent scope set, and $\mathsf{jti}$ is a revocable token identifier.

The gateway verifies $\delta$ on every request and enforces four checks.
First, session binding must match the active session.
Second, caller binding must satisfy the subject pattern.
Third, scope must include the requested intent.
Fourth, the token identifier must not be present in the persistent revocation list.
If any check fails, the gateway fails closed before dispatch.

The effective authorization for each request is an intersection of internal actor capability and external principal capability.
This dual subject rule prevents an external principal from upgrading an internal actor and prevents an internal actor from bypassing external policy constraints.

Ingress federation is modeled as a separate authenticated edge.
The HTTP gateway can require a pinned client certificate hash, a signed request envelope with freshness and nonce replay checks, and a short lived proof token.
When enabled, these ingress claims are bound into request hashing context, so PREVIEW authorization cannot be replayed under a different principal or delegation identifier.
...
We state one primary claim and four compositional properties.
...
No external side effect is committed without dual valid commit evidence bound to the exact request tuple, and each single policy server transcript is simulatable from the allowed leakage function.
...
The contract is specified as a machine checkable assumption and validated by probe assertions.
...
Each allowed leakage term is mapped to explicit code paths and configuration keys to avoid implicit leakage assumptions.
...
\subsection{Official and synthetic channel coverage}

The official benchmark dataset covers channels $C1$ through $C5$ only.
Channels $C6$ and $C7$ are not covered by official cases.
Therefore we evaluate $C6$ and $C7$ using the artifact synthetic channel suite.
We report official and synthetic results separately and we do not mix them in a single rate table.
The reproduction and evidence paths are indexed in \texttt{LEAKAGE\_EVIDENCE.md}.
...
\subsection{Multi agent delegated workflow evaluation}
\label{sec:multiagent-eval}

We run a deterministic multi agent campaign that exercises delegated cross agent messaging and mediated memory access.
The campaign includes seven cases: delegated $C2$ send, delegated $C2$ receive, missing delegation denial, explicit revocation, revoked token denial, dual principal denial for disallowed external intent, and delegated $C5$ mediated memory read.
The campaign implementation is \texttt{scripts/multi\_agent\_federated\_eval.py} and output is \texttt{artifact\_out\_compare/multi\_agent\_federated\_eval.json}.

The current artifact run reports seven of seven expected outcomes, with median latency $0.41$ milliseconds and $p95$ latency $8.28$ milliseconds for gateway level checks.
These results validate that delegated authority and dual principal constraints are enforceable without adding high runtime overhead to the non effectful policy path.
...
The machine checkable contract instance for the artifact is \texttt{spec/secureclaw\_capsule\_contract\_v1.json}.
The contract semantics and downgrade rules are defined in \texttt{capsule/MC\_CONTRACT\_SPEC.md}.
...
The artifact provides cross platform minimal realizations and a smoke test with testable assertions.

\paragraph{Cross platform minimal realizations.}
On macOS the artifact uses \texttt{sandbox-exec} with policy file \texttt{capsule/capsule.sb}.
On Linux the artifact uses \texttt{bubblewrap} with a network namespace, read only binds for system
paths, and a Unix domain socket gateway transport in \texttt{capsule/run\_smoke\_linux.sh}.

\paragraph{Testable assertions.}
The smoke test in \texttt{capsule/smoke.py} produces a structured JSON report.
It asserts that direct reads of a host secret path fail, that direct public network requests fail,
that direct HTTP posts to a local exfiltration endpoint fail when a netless transport is used, that
direct process execution is denied in the macOS capsule configuration, and that a gateway action
request succeeds through the constrained transport.
The artifact includes a contract verifier \texttt{capsule/verify\_contract.py} that checks these
assertions against the machine checkable contract instance \texttt{spec/secureclaw\_capsule\_contract\_v1.json}
and fails closed if any required assertion is violated.

\section{Related work and positioning}
\label{sec:related}

Recent work has established that tool integrated agents are vulnerable to prompt injection and indirect prompt injection.
These attacks can originate from the user, from external content, or from tool outputs, and can steer the runtime toward unauthorized tool use or data exfiltration \citep{greshake2023indirect,injecagent2024,toolselect2025}.
Beyond single step attacks, multi step tool chaining can cause benign tools to compose into dangerous sequences that bypass simple safeguards \citep{stac2025}.

Benchmarks and evaluation suites have been developed to characterize these risks and to enable comparable measurement across systems.
AgentDojo provides a dynamic environment for prompt injection attacks and defenses \citep{agentdojo2024}.
ASB formalizes attacks and defenses for LLM based agents \citep{asb2024}.
AgentLeak focuses on privacy leakage in multi agent systems and defines channel structured leakage targets \citep{agentleak2026}.
AgentHarm evaluates harmful behavior in LLM agents and complements leakage oriented benchmarks with harm oriented measurement \citep{agentharm2024}.

Defense work has proposed runtime monitoring, tool dependency analysis, and end to end security frameworks for agent execution.
AgentArmor enforces policy by program analysis on runtime traces \citep{agentarmor2025}.
IPIGuard defends against indirect prompt injection via tool dependency graphs \citep{ipiguard2025}.
AgentSentinel and ASTRA propose end to end risk and security frameworks for computer use agents and related deployments \citep{agentsentinel2025,astra2025}.

SecureClaw is positioned for a threat model where the runtime and skills are adversarial.
Its core contribution is a non bypassable effect boundary at the executor that verifies dual commit proofs bound to the exact request context.
This differs from policy only approaches that can be bypassed by a compromised runtime and from sandbox only approaches that do not enforce semantic authorization.
SecureClaw also differs from single server policy enforcement designs by preserving query privacy against any single policy server under an explicit leakage function.
Finally, SecureClaw treats skill supply chain ingress as an effectful action protected by the same preview and commit transaction and the same dual proof executor verification.
