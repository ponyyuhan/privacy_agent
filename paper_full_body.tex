...
\paragraph{Practitioner perspective.}
A practitioner perspective has argued that attempts to secure an agent runtime that retains user level powers can be self defeating because the restrictions required for safety also remove the utility of the runtime.\footnote{\url{https://www.aikido.dev/blog/why-trying-to-secure-openclaw-is-ridiculous}}
SecureClaw agrees with this diagnosis for models where the runtime is treated as trusted and effectful.
SecureClaw instead treats the runtime as adversarial and moves enforcement to a small gateway and an executor.
