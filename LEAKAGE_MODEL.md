...
| `C4` tool outputs | `(tool_status, summary_len_bucket, output_handle_ids, timing_bucket)` | tool wrapper response metadata | `gateway/executors/fsexec.py`, `gateway/executors/netexec.py`, `gateway/executors/webhookexec.py` | handleization path, declass policy |
