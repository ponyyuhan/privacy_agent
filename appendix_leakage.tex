...
Let the request binding digest be $h := \mathsf{ReqBind}$ as defined in Appendix \ref{app:proofs:primitives}.
...
Intent-category hiding at this layer requires that observable MPC metadata be fixed across covered
intents, in particular a constant \texttt{program\_id}, constant endpoint class, and constant batch
shape parameters.
...
the tick schedule parameters,
and the multiset of action identifiers included in each tick.
If \texttt{program\_id} or endpoint class varies by intent, that variation is part of
$L_{\mathsf{mpc}}$ and intent-category hiding is not claimed for those deployments.
...
Concretely, with \texttt{PAD\_DFA\_STEPS=1} and cap \texttt{MAX\_DFA\_SCAN\_CHARS}=M, the step count
term becomes deterministic ($n_{\mathsf{confirm}}=M$); otherwise $n_{\mathsf{confirm}}$ may reveal
message-length-correlated shape and is explicitly leaked.
...
With mixers enabled we define per-layer bucket functions:
\[
b_{\mathsf{pir}}(t):=\left\lfloor t/\Delta_{\mathsf{pir}}\right\rfloor,\quad
b_{\mathsf{mpc}}(t):=\left\lfloor t/\Delta_{\mathsf{mpc}}\right\rfloor,
\]
where $\Delta_{\mathsf{pir}}=\texttt{PIR\_MIX\_INTERVAL\_MS}/1000$ and
$\Delta_{\mathsf{mpc}}=\texttt{MPC\_MIX\_INTERVAL\_MS}/1000$.
Then $L_{\mathsf{time}}$ includes the bucket indices, tick counts per window, and public scheduler
configuration.
When a mixer is disabled, fine-grained per-request timing of that layer is treated as additional
leakage and must be declared outside the fixed-shape claim.
...
$C7$ & \texttt{gateway/policy\_unified.py}, \texttt{gateway/capabilities.yaml}, \texttt{capsule/smoke.py} & skill IOC checks, caller capability policy, capsule mediation checks \\
\bottomrule
\end{tabular}
...
For $C7$, the capsule-mediation anchors are
\texttt{capsule/MC\_CONTRACT\_SPEC.md},
\texttt{spec/secureclaw\_capsule\_contract\_v1.json},
\texttt{capsule/smoke.py}, and
\texttt{capsule/verify\_contract.py}.
...
\item Legacy-mode residual only: if keyed request binding is disabled, low-entropy guessing attacks against observable request digests are possible in principle when an attacker can enumerate candidates.
With conditional min-entropy $k$ over candidate inputs and $q_d$ offline trials, residual dictionary advantage is upper-bounded by
\[
\varepsilon_{\mathsf{dict}} \le \min(1,q_d/2^k).
\]
\end{itemize}

\subsection{Evaluation coverage note for channels}

Official AgentLeak cases cover channels $C1$ through $C5$ only.
Channels $C6$ and $C7$ are evaluated with the artifact synthetic full-channel suite, with explicit
evidence index and reproduction commands in \texttt{LEAKAGE\_EVIDENCE.md}.
