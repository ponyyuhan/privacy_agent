...
def _render_md(rep: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# AgentDojo Four-Baseline Fairness Report")
    lines.append("")
    ...
    for sname in ("banking", "slack", "travel", "workspace"):
        ...
        for mode in ("benign", "under_attack"):
            ...
            for base in ("plain", "secureclaw", "ipiguard", "drift"):
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Compare plain/SecureClaw/IPIGuard/DRIFT on fair AgentDojo denominators (benign + under_attack).")
    ...
    out: dict[str, Any] = {
        "status": "OK",
        "benchmark": "AgentDojo-native-four-baseline-fair",
