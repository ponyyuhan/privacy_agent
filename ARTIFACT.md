# SecureClaw Artifact Guide

This repository was previously named MIRAGE-OG++. Some code identifiers still use legacy names for reproducibility and compatibility.
...
- `artifact_out/capsule_contract_verdict.json` (capsule mediation contract verdict + reason codes)
...
If you have `pdflatex` and `bibtex` installed, you can build the paper completely offline:
...
The PDF is written to `artifact_out/paper_build/neurips_2025.pdf`.
...
- The OpenClaw config generated by `scripts/run_openclaw.sh` restricts core tools to a minimal set and exposes **only one side-effect tool**: `secureclaw_act` (alias `mirage_act`), a bridge to the Python SecureClaw MCP gateway.
